...
<body>
<form id="fregisterform" name='joinForm' method="post"  Action='editProc.jsp' onsubmit='return Check_Form()' >

<table width="600px" cellpadding="0" cellspacing="0" align=center>
  <tr><td><img src="../img/editform.gif" border=0></td></tr>
</table>
...
<colgroup>
  ...
  <tr>
    <td class="tle">아이디</td>
	<td class="cont">
	<input  maxlength=20 size=20 id='reg_mb_id' name="id" class="ed" value='<%=user.getId() %>' readonly />
  ...
  <tr>
    <td class="tle">이름</td>
	<td class="cont"><input name="name" placeholder="name" class=ed2":"class=ed" value='<%=user.getName() %>' readonly> <span class='cmt'>* 공백없이 한글만 입력 가능</span>
	</td>
  </tr>
  ...
  <tr>
	...
	<td class="cont">
	<input class=ed type=text id='reg_mb_email' name='email' size=38 maxlength=100  value='<%=user.getEmail() %>'>
	<span id='msg_mb_email'></span>
	</td>
  </tr>
  <tr>
    <td class="tle">생년월일</td>
	<td class="cont"><input class='ed' type='text' id='birth' name='birthday' size='10' maxlength='10' minlength='10' required numeric value='<%=user.getBirthday() %>' title='옆의 달력 아이콘을 클릭하여 날짜를 입력하세요.'></td>
  </tr>
  
  <tr>
    <td class="tle">전화번호</td>
	<td class="cont"><input class=ed type=text name='phone' size=21 maxlength=20 required placeholder='전화번호' value='<%=user.getPhone() %>'></td>
  </tr>
  ...
  <tr>
	...
	<table width="330" border="0" cellspacing="0" cellpadding="0" class="nobd">
	  <tr>
		...
		<input class=ed type=text id="zip" name="zip" value='<%=user.getZip() %>' required >  
		
		<a href="javascript:;" Onclick='DaumPostcode()'><img src="../img/btn_postsearch.gif" border=0 align=absmiddle></a>
		</td>
	  </tr>
	  <tr>
	    <td height="25" colspan="2" class="nobd"><input class=ed type=text id="addr1" name="addr1" size=60 value='<%=user.getAddr1() %>' required ></td>
	  </tr>
	  <tr>
	    <td height="25" colspan="2" class="nobd"><input class=ed type=text id='addr2' name='addr2' size=60 value='<%=user.getAddr2() %>' required ></td>
	  </tr>
	</table>
	</td>
  </tr>
...
<br>
<input type='image' src="../img/editbut.gif" border=0 accesskey='s'> 
</div>
